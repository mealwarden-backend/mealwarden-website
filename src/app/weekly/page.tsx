...
export default function Weekly() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const idx = ds.findIndex((x: any) => (x.dayIndex ?? x.dayOfWeek) === dow)
        const ti = idx >= 0 ? idx : 0
        setSel(ti); setTodayIdx(ti)
  ...
  const toggleMeal = useCallback(async (meal: any) => {
    if (!meal.id || toggling) return
    const wasLogged = logIds.has(meal.id)
    setToggling(meal.id)
    setLogIds(prev => { const next = new Set(prev); if (wasLogged) next.delete(meal.id); else next.add(meal.id); return next })
    try {
      await api.toggleLog({
  ...
  const doneCount = meals.filter(m => logIds.has(m.id)).length
          ...
          <>
            ...
            <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 6, marginBottom: 18 }}>
              {days.map((d: any, i: number) => {
                ...
                const dayLabel = (d.dayName || DAY_SHORT[i] || ('D' + (i + 1))).slice(0, 3)
                return (
                  <button key={i} onClick={() => setSel(i)} style={{
                    flexShrink: 0, padding: '10px 18px', borderRadius: 12,
                    border: '1.5px solid ' + (i === sel ? GREEN : '#e5e7eb'),
                    background: i === sel ? GREEN : '#fff',
                    color: i === sel ? '#fff' : '#374151',
                    fontWeight: 700, fontSize: 13, cursor: 'pointer', fontFamily: FONT, position: 'relative',
                  }}>
                    {dayLabel}
                    {isT && (
                      <span style={{
                        position: 'absolute', top: -6, right: -4, width: 10, height: 10,
                        borderRadius: '50%', background: i === sel ? '#4ade80' : '#16a34a', border: '2px solid #fff',
                      }} />
                    )}
                  </button>
                )
            ...
            {isToday && meals.length > 0 && (
              <div style={{
                ...
                <div>
                  <div style={{ fontFamily: FONT_SYNE, fontSize: 14, fontWeight: 800, color: '#052e16' }}>
                    📍 Today — tap ✓ to log meals
                  </div>
                  <div style={{ fontSize: 12, color: '#6b7280', marginTop: 2 }}>
                    {doneCount}/{meals.length} meals logged · {meals.length > 0 ? Math.round((doneCount / meals.length) * 100) : 0}% done
                  </div>
                </div>
                <div style={{ flex: 1, maxWidth: 180 }}>
                  <div style={{ height: 8, background: '#d1fae5', borderRadius: 4, overflow: 'hidden' }}>
                    <div style={{
                      height: '100%',
                      width: (meals.length > 0 ? Math.round((doneCount / meals.length) * 100) : 0) + '%',
                      background: 'linear-gradient(90deg,#16a34a,#4ade80)', borderRadius: 4, transition: 'width 0.4s ease',
                    }} />
                  </div>
                </div>
              </div>
                ...
                {[
                  ...
                  <div key={t.label} style={{ background: '#fff', borderRadius: 14, padding: '12px 10px', textAlign: 'center', boxShadow: '0 4px 20px rgba(0,0,0,0.05)' }}>
                    <div style={{ fontFamily: FONT_SYNE, fontSize: 18, fontWeight: 800, color: t.ac }}>
                      {t.val}<span style={{ fontSize: 11, color: '#9ca3af', fontFamily: FONT, fontWeight: 600 }}>{t.unit}</span>
                    </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {sections.length === 0 && (
                <div style={{ background: '#fff', borderRadius: 20, padding: 32, textAlign: 'center', color: '#9ca3af', fontSize: 14 }}>
              ...
              {sections.map(sec => (
                <div key={sec} style={{ background: '#fff', borderRadius: 20, overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.06)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '13px 20px', borderBottom: '1px solid #f1f5f9', background: '#fafafa' }}>
                    ...
                    {isToday ? (
                      <span style={{ fontSize: 11, color: GREEN, fontWeight: 700, background: '#f0fdf4', borderRadius: 6, padding: '2px 8px', marginLeft: 'auto' }}>
                        {(grouped[sec] || []).filter(m => logIds.has(m.id)).length}/{(grouped[sec] || []).length} done
                      </span>
                    ) : (
                      <span style={{ fontSize: 12, color: '#9ca3af', marginLeft: 'auto' }}>
                        {(grouped[sec] || []).length} meal{(grouped[sec] || []).length !== 1 ? 's' : ''}
                      </span>
                    )}
                  </div>
                    ...
                    {(grouped[sec] || []).map((m: any, i: number) => {
                      ...
                      return (
                        <div key={i} style={{
                          ...
                          borderBottom: i < (grouped[sec] || []).length - 1 ? '1px solid #f9fafb' : 'none',
                          background: logged ? '#fafffe' : '#fff', transition: 'background 0.2s ease',
                        }}>
                          {isToday && m.id ? (
                            <button onClick={() => toggleMeal(m)} disabled={!!isToggling}
                              style={{
                                width: 32, height: 32, borderRadius: '50%', flexShrink: 0, marginTop: 2,
                                background: logged ? GREEN : '#e5e7eb', border: 'none', color: '#fff', fontSize: 14,
                                cursor: isToggling ? 'default' : 'pointer',
                                display: 'flex', alignItems: 'center', justifyContent: 'center',
                                boxShadow: logged ? '0 2px 8px rgba(22,163,74,0.3)' : 'none',
                              }}>
                              {isToggling ? '…' : logged ? '✓' : '○'}
                            </button>
                          ) : (
                            <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#e5e7eb', flexShrink: 0, marginTop: 8 }} />
                          )}
                          <div style={{ minWidth: 48 }}>
                            <div style={{ fontSize: 12, fontWeight: 700, color: GREEN, marginTop: isToday && m.id ? 8 : 0 }}>{m.timeHHMM || ''}</div>
                          </div>
                          <div style={{ flex: 1 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                              ...
                              {logged && <span style={{ fontSize: 10, fontWeight: 800, color: GREEN, background: '#f0fdf4', borderRadius: 6, padding: '1px 6px' }}>Done ✓</span>}
                            </div>
                          ...
                          <div style={{ textAlign: 'right', flexShrink: 0 }}>
                            <div style={{ fontFamily: FONT_SYNE, fontSize: 15, fontWeight: 800, color: GREEN }}>{cal}</div>
            ...
            {!isToday && (
              <div style={{ textAlign: 'center', marginTop: 20, fontSize: 13, color: '#9ca3af', fontStyle: 'italic', fontFamily: FONT }}>
                Switch to today&apos;s tab to log meals →
              </div>
            )}
          </>
        )}
